...
| 代码注释 | **中文详尽注释**：模块职责、关键步骤、协议边界、为何如此写 |
...
7. **代码注释（强制，中文）**：
   - 注释使用**简体中文**，面向学习可读，避免只有英文标识符而无说明。
   - 文件头说明模块职责与边界（例如：内核禁止厂商协议字段；Adapter 负责协议转换）。
   - 关键类型、公开函数、非显而易见的分支/循环步骤须有注释：做什么、为何、与上下游如何衔接。
   - 协议细节（如 SSE `data:`、`tool_calls` 增量拼接）须注明「协议侧」与「统一类型侧」的对应关系。
   - 禁止无信息注释（如「定义变量」）；允许并鼓励解释学习要点与安全注意（密钥、脱敏、校验时机）。
...
3. ~~进入 M1。~~ 已通过 V1/V2；~~M2–M5~~ **M2–M5 已实现**；本地勾选 V3–V7 后进入 **M6**。
